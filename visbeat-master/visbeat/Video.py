...
from visbeat.AObject import *
from visbeat.TimeSignal import *
from visbeat.Audio import *
from visbeat.Warp import *
from visbeat.Image import *
...
def MPYWriteVideoFile(mpyclip, filename, **kwargs):
    ...
    temp_audio_filename = get_temp_file_path(final_file_path=b'TEMP_'+filename+b'.m4a', temp_dir_path=Video.VIDEO_TEMP_DIR);
    if(isinstance(temp_audio_filename,str)):
        temp_audio_filename = temp_audio_filename.encode()
    print(filename.decode())
    return mpyclip.write_videofile(filename=filename.decode(), temp_audiofile= temp_audio_filename.decode(), audio_codec='aac', **kwargs);
...
class Video(TimeSignal):
    ...
    def getMPYClip(self, get_audio=True):
        ...
        path = self.getPath()
        return mpy.VideoFileClip(path, audio=get_audio);
    ...
    def openVideoWriter(self, output_file_path, fps=None):
        ...
        if(isinstance (output_file_path,bytes)):
            output_file_path = output_file_path.decode()
...
import visbeat.Video_CV as Video_CV;
...
from visbeat.vbgui import BeatGUI
