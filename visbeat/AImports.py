#AImports
import visbeat.ADefines as defines
import os
import os.path
import errno
import json
...
def get_temp_file_path(final_file_path="TEMP", temp_dir_path = None):
    if(isinstance(final_file_path,bytes)):
        final_file_path = final_file_path.decode()
    ...
    temp = pathstring(destfolder+tempname)
    return temp.encode()
